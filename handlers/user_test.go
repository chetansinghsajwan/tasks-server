...
const (
	validUserID_0          = "user-0"
	validUserPass_0        = "password-0"
	validUserEmail_0       = "user0@domain.com"
	validUserFullName_0    = "First Middle Last"
	validUserDisplayName_0 = "User0"

	validUserID_1          = "user-1"
	validUserPass_1        = "password-1"
	validUserEmail_1       = "user1@domain.com"
	validUserFullName_1    = "First Middle Last"
	validUserDisplayName_1 = "User1"

	validUserID_2 = "user-2"
)
...
func TestUserHandler(t *testing.T) {
	...
	t.Run("Update user with invalid ids", func(t *testing.T) {
		...
		for _, invalidID := range invalidIDs {
			...
			t.Run(fmt.Sprintf("Update user with invalid id '%s'", invalidID), func(t *testing.T) {
				...
				var req = httptest.NewRequest("PATCH", fmt.Sprintf("/api/users/%s", validUserID_0), bytes.NewBufferString(reqBody))
				req.Header.Set("Content-Type", "application/json")
				...
				var resBody UpdateUserResponse
				if err := json.Unmarshal(rec.Body.Bytes(), &resBody); err != nil {
					t.Fatalf("Failed to parse response body, err: %v, body: %s", err, rec.Body.String())
				}
	...
	t.Run("Update user with invalid emails", func(t *testing.T) {
		...
		for _, invalidEmail := range invalidEmails {
			...
			t.Run(fmt.Sprintf("Update user with invalid email '%s'", invalidEmail), func(t *testing.T) {
	...
	t.Run("Update user with invalid full names", func(t *testing.T) {
		...
		for _, invalidFullName := range invalidFullNames {
			...
			t.Run(fmt.Sprintf("Update user with invalid full name '%s'", invalidFullName), func(t *testing.T) {
	...
	t.Run("Update user with invalid display names", func(t *testing.T) {
		...
		for _, invalidDisplayName := range invalidDisplayNames {
			...
			t.Run(fmt.Sprintf("Update user with invalid display name '%s'", invalidDisplayName), func(t *testing.T) {
	...
	t.Run("Update user with invalid password", func(t *testing.T) {
		...
		for _, invalidPassword := range invalidPasswords {
			...
			t.Run(fmt.Sprintf("Update user with invalid password '%s'", invalidPassword), func(t *testing.T) {
	...
	t.Run("Update user with valid data", func(t *testing.T) {
	...
	t.Run("Update user that doesn't exist", func(t *testing.T) {
		...
		var req = httptest.NewRequest("PATCH", fmt.Sprintf("/api/users/%s", validUserID_2), bytes.NewBufferString(reqBody))
		req.Header.Set("Content-Type", "application/json")
	...
	t.Run("Get user that exists", func(t *testing.T) {
		...
		if resBody.User.ID != validUserID_1 {
			t.Fatalf("Expected user id '%s', got '%s'", validUserID_1, resBody.User.ID)
		}

		if resBody.User.Email != validUserEmail_1 {
			t.Fatalf("Expected user email '%s', got '%s'", validUserEmail_1, resBody.User.Email)
		}

		if resBody.User.FullName != validUserFullName_1 {
			t.Fatalf("Expected user full name '%s', got '%s'", validUserFullName_1, resBody.User.FullName)
		}

		if resBody.User.DisplayName == nil {
			t.Fatalf("Expected user display name '%s', got 'nil'", validUserDisplayName_1)
		}

		if *resBody.User.DisplayName != validUserDisplayName_1 {
			t.Fatalf("Expected user display name '%s', got '%s'", validUserDisplayName_1, *resBody.User.DisplayName)
		}
	})

	t.Run("Get user that doesn't exist", func(t *testing.T) {

		var req = httptest.NewRequest("GET", fmt.Sprintf("/api/users/%s", validUserID_2), nil)
		var rec = httptest.NewRecorder()
	...
	t.Run("Delete user that exists", func(t *testing.T) {
		...
		var req = httptest.NewRequest("DELETE", fmt.Sprintf("/api/users/%s", validUserID_1), nil)
		var rec = httptest.NewRecorder()
